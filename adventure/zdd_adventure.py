...
class ZDDAdventure:
    ...
    def create_floors(self):
        ...
        small_key = Item("small key", "a small key, what can it open though...?", movable=True)
        archive_room = Room("archive", "Old records and dusty books everywhere.", [analog_book, small_key])
        cellar.add_room("archive", archive_room)
        cellar.add_room("toilet", ALL_ROOMS["toilet_cellar"])

        # -------------------------------
        # ... Add other rooms ...
        first_floor.add_room("closet", ALL_ROOMS["janitors_closet"])
